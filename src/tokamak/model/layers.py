...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, positions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Return ``(cos, sin)`` tables of shape ``[len(positions), head_dim]``."""
...
def apply_rotary_emb(
    ...
    Args:
        ...
        cos: Cosine table of shape ``[seq_len, head_dim]`` (float32).
        sin: Sine table of shape ``[seq_len, head_dim]`` (float32).
    """
